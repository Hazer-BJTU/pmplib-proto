...
std::weak_ptr<GraphNode> ThreadPool::grab() {
    ...
    content.wait(lock1, [this]() -> bool {return !this->q.empty();});
    std::unique_lock<std::mutex> lock2(conditions_lock);
    if (end_flag) {
        return std::weak_ptr<GraphNode>();
    } else {
        std::weak_ptr<GraphNode> node_ptr = q.front();
        q.pop();
        space.notify_one();
        return node_ptr;
    }
...
void ThreadPool::set(int idx) {
    std::unique_lock<std::mutex> lock(conditions_lock);
