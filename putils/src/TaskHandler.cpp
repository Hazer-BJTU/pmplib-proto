            ...
            workers.emplace_back([this]() {
                while(true) {
                    if (state.load(std::memory_order_acquire) == INACTIVE) {
                        std::unique_lock<std::mutex> lock(cv_inactive_lock);
                        active_workers.fetch_sub(1, std::memory_order_acq_rel);
                        cv_all_done.notify_all();
                        cv_inactive.wait(lock, [this]() -> bool { return state.load(std::memory_order_acquire) == ACTIVE; });
...
void TaskHandler::wait_all_done() noexcept {
    std::unique_lock<std::mutex> lock(cv_all_done_lock);
    cv_all_done.wait(lock, [this]() -> bool { return task_queue->empty(); });
    inactivate();
